...
// v0.11.6.7 // LEAN LIVE GLOBAL GLOBE
...
const eventPanel=$('#eventPanel'),devPanel=$('#devPanel');if(eventPanel){eventPanel.open=true;$('#utilityEventHost')?.appendChild(eventPanel)}if(devPanel){devPanel.open=false;$('#utilityDevHost')?.appendChild(devPanel)}applyVisualSettings();applyAudioSettings();cpRefreshCampaign();cpRefreshCampaign02();setArcadeOA(arcadeOA);cpShowMenu();(function bootSequence(){const splash=$('#bootSplash'),bar=$('#bootLoadBar'),pct=$('#bootPct'),minBoot=1800,t0=performance.now();try{backgroundMusic.volume=.34*audioLevels.music*(masterMuted?0:1);const p=backgroundMusic.play();if(p&&p.catch)p.catch(()=>{})}catch(e){}const critical=[GLOBAL_EARTH_TEXTURE_SRC,'assets/maps/global-deep-space.png'];const planningEntries=Object.entries(GLOBAL_MAP_IMAGE);let complete=0,total=critical.length+planningEntries.length+1;function paint(){const n=Math.min(99,Math.round(complete/total*100));if(bar){bar.style.width=n+'%';bar.style.backgroundColor=n<50?'#ff4b45':n<80?'#e0b23d':'#55d67a'}if(pct){pct.textContent=n+'%';pct.style.color=n<50?'#ff6b65':n<80?'#e0b23d':'#55d67a'}}function preload(src){return new Promise(resolve=>{const img=new Image();let done=false;const finish=()=>{if(done)return;done=true;complete++;paint();resolve()};img.onload=()=>{const d=img.decode?.();if(d&&d.then)d.then(finish).catch(finish);else finish()};img.onerror=finish;img.src=src;if(img.complete&&img.naturalWidth)img.onload()})}async function warm(){paint();const deck=$('#planningWarmDeck');const planningLoads=planningEntries.map(([oa,src])=>new Promise(resolve=>{const img=ensurePlanningRaster(oa);if(deck&&img.parentNode!==deck)deck.appendChild(img);let done=false;const finish=()=>{if(done)return;done=true;complete++;paint();resolve()};const decoded=()=>{const d=img.decode?.();if(d&&d.then)d.then(finish).catch(finish);else finish()};if(img.complete&&img.naturalWidth)decoded();else{img.onload=decoded;img.onerror=finish}}));await Promise.all(critical.map(preload));const planningBudget=new Promise(resolve=>setTimeout(resolve,900));Promise.race([Promise.allSettled(planningLoads),planningBudget]).then(()=>{if(deck){void deck.offsetWidth;requestAnimationFrame(()=>requestAnimationFrame(()=>{}))}});try{const gl=initGlobalSurfaceGL();const limit=performance.now()+2200;while(gl&&!gl.ready&&performance.now()<limit)await new Promise(r=>setTimeout(r,32));if(gl){drawGlobalSurfaceGL(8,8,3);try{gl.gl.finish()}catch(e){}}}catch(e){}complete++;paint();const wait=Math.max(0,minBoot-(performance.now()-t0));if(wait)await new Promise(r=>setTimeout(r,wait));if(bar){bar.style.width='100%';bar.style.backgroundColor='#55d67a'}if(pct){pct.textContent='100%';pct.style.color='#55d67a'}requestAnimationFrame(()=>requestAnimationFrame(()=>splash?.classList.add('done')))}warm()})();
