...
B.forEach(b=>{b.exposure=0;b.degraded=false})}
...
if(campaignMission==='01-03'||campaignMission==='01-04'||campaignMission==='01-05'){
  const siteList=B.map(b=>({name:b.id,x:b.x,y:b.y}));
  if(siteList.length&&Math.random()<(campaignMission==='01-05'?.70:.58)){let ps=siteList[Math.floor(Math.random()*siteList.length)];t.proxSite=ps.name;t.x=ps.x+(Math.random()-.5)*105;t.y=ps.y+(Math.random()-.5)*105;t.proxSince=performance.now();t.stationary=Math.random()<.62}
...
pct=100;pulse(col.type);ensureRing(col);log('CLASSIFIED // '+designation(col)+' // '+col.type);col=null;if(campaignMission==='01-02')cpCheck0102();if(campaignMission==='01-03')cpCheck0103();ui(true)},1000)}function cpDecisionFloat(t,ok){
...
function decide(a){
 ...
 if(ok){
   score+=100;
   if(campaignMission==='01-01'||campaignMission==='01-02'||campaignMission==='01-03'||campaignMission==='01-04'||campaignMission==='01-05'){
     campaignCorrect++;
...
function cpThreatMission(){return campaignMission==='01-04'||campaignMission==='01-05'}
...
function cpIntelDropOffer(){
 ...
 log('INTEL DROP // NEW REPORTING AVAILABLE');cpNotice('INTEL DROP','NEW REPORTING AVAILABLE','intel');
}
function cpIntelDropChoose(kind){
 ...
 if(kind==='MITIGATE'){
   ...
   log('INTEL DROP // ISR FUSION ACTIVE // 20 SEC');cpNotice('INTEL DROP // EXPLOIT','ISR FUSION ACTIVE // 20 SEC','intel');
 }
}
function cpIntelDropTick(){
 ...
 if(intelDropActive){
   ...
   if(now>=intelDropExpires){intelDropActive=false;const d=$('#intelDrop');if(d)d.classList.add('hidden');log('INTEL DROP EXPIRED');cpNotice('INTEL DROP','REPORT EXPIRED','intel');}
 }
 if(intelFusionUntil){
...
function cpCompromiseFloor(){return Math.min(50,B.reduce((sum,b)=>sum+(b.exposure||0),0)/10)}
...
function cpContestedTick(dt){
 ...
 T.forEach(t=>{
   ...
   if(threat&&t.proxSite&&!t.dead&&!t.done){
     ...
     if(performance.now()-t.collectStarted>5000)t.hostileCollect=true;
     if(t.hostileCollect){
       active++;
   ...
   pendingAdds.forEach(([t,site,requested])=>{
     ...
     if(site&&!site.degraded){
       ...
       if(site.exposure>=100){
         site.exposure=100;site.degraded=true;
         const floor=cpApplyCompromiseFloor();
         log(site.id+' // FULLY EXPOSED // BASE DEGRADED // OE FLOOR '+floor+'%');
         cpNotice(site.id+' // DEGRADED','OE FLOOR '+floor+'% // TRACKS MOBILIZING','hostile');
 ...
 if(campaignCompromise>=60&&!campaignFinished)cpFinish0104('COMPROMISE');
}
...
function enemy(dt){if(campaignMission==='01-02'||campaignMission==='01-03')return;T.forEach(t=>{if(t.done||(t.truth!=='HOSTILE'&&t.truth!=='IRREGULAR'))return;B.forEach(b=>{if(D(t,b)<=t.enemyRange){let mult=t.truth==='IRREGULAR'?2.15:1,burn=(t.stationary?1.8:1.05)*mult*dt;b.exposure=Math.min(100,b.exposure+burn);score-=burn*.12}})})}function detail(){if(!sel||sel.done){$('#none').hidden=false;$('#detail').hidden=true;return}$('#none').hidden=true;$('#detail').hidden=false;let d=D(A,sel),inside=d<=cpEffectiveISRRadius(),u=sel.type==='UNKNOWN';$('#tid').textContent=designation(sel);$('#type').textContent=sel.type;$('#motion').textContent=sel.stationary?'STATIONARY':'MOBILE';
...
if(ot)ot.style.display=(campaignMission&&['01-01','01-02','01-03','01-04','01-05'].includes(campaignMission))?'flex':'none';
if(campaignMission&&or){
 ...
 or.textContent=campaignCorrect+'/'+resolveTarget;
 ...
 og.textContent=Math.round(integrity)+'/80';
 ...
 or.parentElement.classList.toggle('complete',campaignCorrect>=resolveTarget);
...
function cpCampaignMenu(){cpRefreshCampaign();window.CP_MENU=true;window.CP_PAUSED=false;running=false;stopCollect();$('#mainMenu').classList.add('hidden');$('#pauseMenu').classList.add('hidden');$('#briefing0101').classList.add('hidden');$('#briefing0102').classList.add('hidden');$('#briefing0103').classList.add('hidden');$('#briefing0104').classList.add('hidden');$('#briefing0105').classList.add('hidden');$('#campaignResult').classList.add('hidden');$('#campaignMenu').classList.remove('hidden')}
...
function cpBegin0105(){campaignMission='01-05';campaignIds=0;campaignCorrect=0;campaignThreatResolved=0;campaignNonThreatResolved=0;campaignStationaryAssessed=0;campaignStationarySpawned=0;campaignProximityAssessed=0;campaignCollectionInterrupted=0;campaignCompromise=0;campaignFinished=false;cpIntelDropReset();cpHideCampaignOverlays();let w=$('#hostileCollectionWarning');if(w)w.classList.add('hidden');window.CP_MENU=false;window.CP_PAUSED=false;$('#startScreen').style.display='none';deploy();last=performance.now()}
function cpBegin0104(){campaignMission='01-04';campaignIds=0;campaignCorrect=0;campaignThreatResolved=0;campaignNonThreatResolved=0;campaignStationaryAssessed=0;campaignStationarySpawned=0;campaignProximityAssessed=0;campaignCollectionInterrupted=0;campaignCompromise=0;campaignFinished=false;cpIntelDropReset();cpHideCampaignOverlays();let w=$('#hostileCollectionWarning');if(w)w.classList.add('hidden');window.CP_MENU=false;window.CP_PAUSED=false;$('#startScreen').style.display='none';deploy();last=performance.now()}

function cpBegin0103(){campaignMission='01-03';campaignIds=0;campaignCorrect=0;campaignThreatResolved=0;campaignNonThreatResolved=0;campaignStationaryAssessed=0;campaignStationarySpawned=0;campaignProximityAssessed=0;campaignFinished=false;cpHideCampaignOverlays();window.CP_MENU=false;window.CP_PAUSED=false;$('#startScreen').style.display='none';deploy();last=performance.now()}

function cpBegin0102(){campaignMission='01-02';campaignIds=0;campaignCorrect=0;campaignThreatResolved=0;campaignNonThreatResolved=0;campaignStationaryAssessed=0;campaignStationarySpawned=0;campaignFinished=false;cpHideCampaignOverlays();window.CP_MENU=false;window.CP_PAUSED=false;$('#startScreen').style.display='none';deploy();last=performance.now()}
