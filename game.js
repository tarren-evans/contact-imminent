(()=>{function ciPhysicalPhoneMapFit(){const m=document.getElementById('map'),w=document.getElementById('mapWrap');if(!m||!w)return;m.setAttribute('viewBox','0 0 900 650');const phone=window.innerWidth>680&&window.innerHeight<=500;if(!phone){m.style.removeProperty('width');m.style.removeProperty('height');m.style.removeProperty('left');m.style.removeProperty('right');m.style.removeProperty('top');m.style.removeProperty('bottom');return;}const cs=getComputedStyle(w),sl=parseFloat(cs.getPropertyValue('--phone-safe-left'))||8,sr=parseFloat(cs.getPropertyValue('--phone-safe-right'))||8,st=parseFloat(cs.getPropertyValue('--phone-safe-top'))||8,sb=parseFloat(cs.getPropertyValue('--phone-safe-bottom'))||8,aw=Math.max(1,w.clientWidth-sl-sr),ah=Math.max(1,w.clientHeight-st-sb),scale=Math.min(aw/900,ah/650),mw=Math.floor(900*scale),mh=Math.floor(650*scale);m.style.width=mw+'px';m.style.height=mh+'px';m.style.left=(sl+Math.max(0,(aw-mw)/2))+'px';m.style.right='auto';m.style.top=(st+Math.max(0,(ah-mh)/2))+'px';m.style.bottom='auto';}ciPhysicalPhoneMapFit();window.addEventListener('resize',ciPhysicalPhoneMapFit,{passive:true});window.visualViewport?.addEventListener('resize',ciPhysicalPhoneMapFit,{passive:true});const $=s=>document.querySelector(s),NS='http://www.w3.org/2000/svg',D=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y),H=(x,y)=>(Math.atan2(x,-y)*180/Math.PI+360)%360;function renderGridExtensions(){const g=$('#gridExtensions');if(!g)return;const w=Math.max(1,g.clientWidth),h=Math.max(1,g.clientHeight),scale=Math.min(w/900,h/650),padX=(w-900*scale)/2,padY=(h-650*scale)/2,rightEdge=padX+900*scale,r=52*scale,dx=78*scale,dy=Math.sqrt(3)*52*scale;g.setAttribute('viewBox',`0 0 ${w} ${h}`);g.setAttribute('preserveAspectRatio','none');g.textContent='';if(padX<.5)return;const defs=document.createElementNS(NS,'defs'),clip=document.createElementNS(NS,'clipPath');clip.setAttribute('id','gridGutterClip');const left=document.createElementNS(NS,'rect');left.setAttribute('x','0');left.setAttribute('y','0');left.setAttribute('width',padX);left.setAttribute('height',h);const right=document.createElementNS(NS,'rect');right.setAttribute('x',rightEdge);right.setAttribute('y','0');right.setAttribute('width',Math.max(0,w-rightEdge));right.setAttribute('height',h);clip.append(left,right);defs.appendChild(clip);g.appendChild(defs);const layer=document.createElementNS(NS,'g');layer.setAttribute('clip-path','url(#gridGutterClip)');g.appendChild(layer);const points=(cx,cy)=>Array.from({length:6},(_,i)=>{const a=Math.PI/180*(60*i);return (cx+r*Math.cos(a)).toFixed(2)+','+(cy+r*Math.sin(a)).toFixed(2)}).join(' ');const baseX=padX-156*scale,baseY=padY-dy,cMin=Math.floor((0-baseX-r)/dx)-2,cMax=Math.ceil((w-baseX+r)/dx)+2;for(let col=cMin;col<=cMax;col++){const cx=baseX+col*dx,offset=((col%2)+2)%2*dy/2,rowMin=Math.floor((0-(baseY+offset)-r)/dy)-2,rowMax=Math.ceil((h-(baseY+offset)+r)/dy)+2;for(let row=rowMin;row<=rowMax;row++){const poly=document.createElementNS(NS,'polygon');poly.setAttribute('points',points(cx,baseY+offset+row*dy));poly.setAttribute('class','geoHex');layer.appendChild(poly)}}}let gridExtensionRAF=0;function scheduleGridExtensions(){cancelAnimationFrame(gridExtensionRAF);gridExtensionRAF=requestAnimationFrame(renderGridExtensions)};scheduleGridExtensions();window.addEventListener('resize',scheduleGridExtensions,{passive:true});window.visualViewport?.addEventListener('resize',scheduleGridExtensions,{passive:true});function renderTerrain(){
 ...
 // v0.6.6.14: terrain imagery lives on #mapWrap as overscan. The SVG now
 ...
 if(grid){
   ...
   let col=0;
   for(let cx=-2*dx;cx<=900+2*dx;cx+=dx,col++){
