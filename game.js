...
log('MISSION START // CONTACT IMMINENT 0.11.5.16 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(brand)brand.textContent='v0.11.5.16 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpRenderStrategicPriorityNAI();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();requestAnimationFrame(()=>window.ciPhysicalPhoneMapFit?.())});}
...
// v0.11.5.16 // GLOBAL GLOBE FOUNDATION
...
// v0.11.5.16 // MOBILE GLOBAL PERFORMANCE PROFILE
...
const globalFx={lights:true,clouds:true,aurora:true,markers:true};
...
// v0.11.5.16 // GLOBAL EARTH FOUNDATION
...
function drawGlobalGlobe(now){drawGlobalSpace(now);const c=$('#globalGlobeCanvas'),wrap=$('#globalEarthWrap'),ov=$('#globalGlobeOverlay');if(!c||!wrap||!ov)return;initGlobalGlobe();const rect=wrap.getBoundingClientRect(),dpr=GLOBAL_RENDER_DPR,logical=Math.max(2,Math.round(Math.min(rect.width,rect.height)*dpr)),pad=1.72,W=Math.round(logical*pad),H=Math.round(logical*pad);if(c.width!==W||c.height!==H){c.width=W;c.height=H}const ctx=c.getContext('2d'),cx=W/2,cy=H/2,r=logical*.40*globalGlobe.zoom;ctx.clearRect(0,0,W,H);drawGlobalSurfaceGL(W,H,r);ctx.save();ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.clip();
 ...
 if(globalFx.lights){ const cityCenters=[[43,-121,1],[40,-111,.85],[36,-96,1.25],[31,-82,.82],[22,-62,.95],[8,-49,.78],[-13,-45,.72],[-27,-57,.92],[50,-7,.9],[46,18,1.28],[38,34,1.12],[30,49,.96],[20,61,.84],[9,73,1.18],[-4,86,.88],[-16,101,.72],[38,111,1.38],[29,124,1.15],[16,137,.96],[-5,148,.78],[-23,143,.88],[-35,132,.7],[63,25,.62],[70,41,.48],[56,68,.76],[43,78,.92],[26,91,1.06],[5,113,1.12],[-7,18,.66],[-19,29,.74],[-32,41,.82],[13,-12,.7],[1,-3,.64],[-23,-5,.58]];
 ...
 if(globalFx.clouds){ // GLOBAL WEATHER: broad cloud decks, fronts, and propagating storm systems.
 ...
 if(globalFx.aurora){  // Polar aurora: broad luminous vertical curtains, geographically attached.
 ctx.save();ctx.globalCompositeOperation='screen';
...
function syncGlobalPerfUI(){
 [['globalFxLights','lights'],['globalFxClouds','clouds'],['globalFxAurora','aurora'],['globalFxMarkers','markers']].forEach(([id,k])=>{const el=$('#'+id);if(el)el.checked=globalFx[k]});
 const bare=$('#globalBareGlobe');if(bare)bare.classList.toggle('active',!globalFx.lights&&!globalFx.clouds&&!globalFx.aurora&&!globalFx.markers);
 const ov=$('#globalGlobeOverlay');if(ov)ov.style.visibility=globalFx.markers?'visible':'hidden';
}
[['globalFxLights','lights'],['globalFxClouds','clouds'],['globalFxAurora','aurora'],['globalFxMarkers','markers']].forEach(([id,k])=>$('#'+id)?.addEventListener('change',e=>{globalFx[k]=e.target.checked;syncGlobalPerfUI()}));
$('#globalBareGlobe')?.addEventListener('click',()=>{const any=globalFx.lights||globalFx.clouds||globalFx.aurora||globalFx.markers;Object.keys(globalFx).forEach(k=>globalFx[k]=!any);syncGlobalPerfUI()});
syncGlobalPerfUI();
