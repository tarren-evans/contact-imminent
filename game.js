...
log('MISSION START // CONTACT IMMINENT 0.11.6.0 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(brand)brand.textContent='v0.11.6.0 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpRenderStrategicPriorityNAI();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();requestAnimationFrame(()=>window.ciPhysicalPhoneMapFit?.())});}
...
// v0.11.6.0 // GLOBAL GLOBE FOUNDATION
...
// v0.11.6.0 // MOBILE GLOBAL PERFORMANCE PROFILE
...
// v0.11.6.0 // GLOBAL EARTH FOUNDATION
...
const GLOBAL_GLOBE_FRAME_COUNT=360;
const GLOBAL_GLOBE_FRAME_PATH=n=>'assets/maps/global-globe-frames/g-'+String(((n%GLOBAL_GLOBE_FRAME_COUNT)+GLOBAL_GLOBE_FRAME_COUNT)%GLOBAL_GLOBE_FRAME_COUNT).padStart(3,'0')+'.webp';
const globalFrameCache=new Map();
function getGlobalGlobeFrame(n){n=((n%GLOBAL_GLOBE_FRAME_COUNT)+GLOBAL_GLOBE_FRAME_COUNT)%GLOBAL_GLOBE_FRAME_COUNT;if(globalFrameCache.has(n))return globalFrameCache.get(n);const im=new Image();im.decoding='async';im.src=GLOBAL_GLOBE_FRAME_PATH(n);globalFrameCache.set(n,im);if(globalFrameCache.size>9){for(const [k] of globalFrameCache){let d=Math.abs(k-n);d=Math.min(d,GLOBAL_GLOBE_FRAME_COUNT-d);if(d>4){globalFrameCache.delete(k);break}}}return im}
function warmGlobalFrames(n){[-2,-1,0,1,2].forEach(d=>getGlobalGlobeFrame(n+d))}
function drawGlobalGlobe(now){drawGlobalSpace(now);const c=$('#globalGlobeCanvas'),wrap=$('#globalEarthWrap'),ov=$('#globalGlobeOverlay');if(!c||!wrap||!ov)return;initGlobalGlobe();const rect=wrap.getBoundingClientRect(),dpr=GLOBAL_RENDER_DPR,logical=Math.max(2,Math.round(Math.min(rect.width,rect.height)*dpr)),pad=1.72,W=Math.round(logical*pad),H=Math.round(logical*pad);if(c.width!==W||c.height!==H){c.width=W;c.height=H}const ctx=c.getContext('2d'),cx=W/2,cy=H/2,r=logical*.40*globalGlobe.zoom;ctx.clearRect(0,0,W,H);
 const frame=((Math.round(-globalGlobe.yaw)%360)+360)%360,im=getGlobalGlobeFrame(frame);warmGlobalFrames(frame);if(im.complete&&im.naturalWidth){const tilt=Math.max(-18,Math.min(18,globalGlobe.pitch));ctx.save();ctx.translate(cx,cy+tilt*r*.004);ctx.rotate(tilt*Math.PI/180*.045);ctx.drawImage(im,-r,-r,r*2,r*2);ctx.restore()}
 const rate=Number(getComputedStyle(document.documentElement).getPropertyValue('--global-rate')||1),dt=Math.min(.05,(now-globalGlobe.lastT)/1000);globalGlobe.lastT=now;if(!globalGlobe.drag)globalGlobe.yaw-=dt*6*rate;globalGlobe.orbitT+=dt*rate;
 const ovRect=ov.getBoundingClientRect(),sx=ovRect.width/W,sy=ovRect.height/H;Object.entries(GLOBAL_OA_GEO).forEach(([oa,ll])=>{let q=globeProject(ll.lat,ll.lon,cx,cy,r),el=ov.querySelector('[data-oa="'+oa+'"]');if(el){el.style.left=(q.x*sx)+'px';el.style.top=(q.y*sy)+'px';el.classList.toggle('callout-left',q.x<cx);el.classList.toggle('callout-right',q.x>=cx);el.classList.toggle('backside',q.z<=0);el.classList.toggle('selected',globalSelectedOA===oa)}});GLOBAL_ORBITS.forEach((o,i)=>{let ll=globeOrbitLatLon(o,globalGlobe.orbitT),q=globeProject(ll.lat,ll.lon,cx,cy,r),el=ov.querySelector('.globe-sat-'+i);if(el){el.style.left=(q.x*sx)+'px';el.style.top=(q.y*sy)+'px';el.classList.toggle('backside',q.z<=0)}})}
