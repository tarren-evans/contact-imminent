...
log('MISSION START // CONTACT IMMINENT 0.11.5.13 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(brand)brand.textContent='v0.11.5.13.1 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpRenderStrategicPriorityNAI();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();requestAnimationFrame(()=>window.ciPhysicalPhoneMapFit?.())});}
...
// v0.11.5.13.1 // GLOBAL GLOBE FOUNDATION
...
function drawGlobalSpace(now){
 ...
 const rect=scene.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1),W=Math.max(2,Math.round(rect.width*dpr)),H=Math.max(2,Math.round(rect.height*dpr));
...
// v0.11.5.13.1 // GLOBAL EARTH FOUNDATION
...
function drawGlobalGlobe(now){drawGlobalSpace(now);const c=$('#globalGlobeCanvas'),wrap=$('#globalEarthWrap'),ov=$('#globalGlobeOverlay');if(!c||!wrap||!ov)return;initGlobalGlobe();const rect=wrap.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1),logical=Math.max(2,Math.round(Math.min(rect.width,rect.height)*dpr)),pad=1.72,W=Math.round(logical*pad),H=Math.round(logical*pad);if(c.width!==W||c.height!==H){c.width=W;c.height=H}const ctx=c.getContext('2d'),cx=W/2,cy=H/2,r=logical*.40*globalGlobe.zoom;ctx.clearRect(0,0,W,H);drawGlobalSurfaceGL(W,H,r);ctx.save();ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.clip();
 ...
 fronts.forEach((f,fi)=>{for(let k=0;k<52;k++){const u=k/51-.5,lon=f.lon+u*f.len+weatherT*f.spd*360,lat=f.lat+u*f.len*f.tilt+Math.sin(k*.72+weatherT*11+fi)*2.4;cloudPuff(lat,lon,.013+(k%5)*.0019,f.a,fi*100+k);if(k%2===0)cloudPuff(lat+Math.sin(k*.9)*3.1,lon+Math.cos(k*.7)*3.8,.010+(k%3)*.0017,f.a*.72,fi*500+k);}});
 ...
 storms.forEach((st,si)=>{
   ...
   for(let arm=0;arm<4;arm++)for(let k=0;k<38;k++){
   ...
   for(let k=0;k<28;k++){const a=k*2.399963+weatherT*9,rad=Math.sqrt((k+.4)/28)*4.6*st.scale;cloudPuff(st.lat+Math.sin(a)*rad,centerLon+Math.cos(a)*rad,.0105+(k%3)*.0018,.20,si*2000+k);}
 });
...
function globalGlobeFrame(now){if(!$('#globalMenu')?.classList.contains('hidden'))drawGlobalGlobe(now);requestAnimationFrame(globalGlobeFrame)} requestAnimationFrame(globalGlobeFrame);
