...
log('MISSION START // CONTACT IMMINENT 0.11.6.34 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
let verdantTickAccum=0;
function verdantSmartTick(dt){if(!verdantSmartActive||activeOA!=='VERDANT'){verdantTickAccum=0;return}verdantTickAccum+=dt;if(verdantTickAccum<.10)return;const step=verdantTickAccum;verdantTickAccum=0;const cfg=window.CP_VERDANT_MAP?.routeFoundation;if(!cfg)return;let routesInRange=0,visualChanged=false;const radius=118,r2=radius*radius;
 cfg.routes.forEach(r=>{verdantEnsureCells(r);let hit=false;const before=verdantStateFor(r);for(const c of r.cells){const dx=c.x-A.x,dy=c.y-A.y;if(dx*dx+dy*dy<=r2){hit=true;c.progress+=step}}if(hit)routesInRange++;const after=verdantStateFor(r);if(after!==before){r.state=after;visualChanged=true;const m=verdantRouteMetrics(r);cpNotice('ROUTE INTELLIGENCE // '+r.id,after.toUpperCase()+' // '+Math.round(m.coverage*100)+'% COVERAGE','intel');log('VERDANT // '+r.id+' // '+after.toUpperCase()+' // '+Math.round(m.coverage*100)+'%')}});
...
function cpVerdantHexAspect(){
 ...
 const asset=$('#asset');if(asset)asset.dataset.verdantAspect=String(k);
 for(const b of B){if(b?.el){b.el.dataset.verdantAspect=String(k);b.el.setAttribute('transform',`translate(${b.x} ${b.y}) scale(${k} 1)`)}}
}
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small'),map=$('#map');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(map)map.setAttribute('preserveAspectRatio',activeOA==='VERDANT'?'none':'xMidYMid meet');if(brand)brand.textContent='v0.11.6.34 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpVerdantRaster();cpRenderStrategicPriorityNAI();cpRenderVerdantRoutes();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();cpVerdantHexAspect();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();cpVerdantHexAspect()})});}
