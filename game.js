...
log('MISSION START // CONTACT IMMINENT 0.11.6.32 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function verdantEnsureCells(r){
 ...
 for(const p of (r.points||[])){const gx=Math.round(p[0]/20),gy=Math.round(p[1]/20),k=gx+','+gy;if(seen.has(k))continue;seen.add(k);r.cells.push({x:p[0],y:p[1],progress:0,state:'unassessed'})}
}
...
function cpRenderVerdantRoutes(){
 ...
 const cfg=window.CP_VERDANT_MAP?.routeFoundation;if(!cfg)return;const ns='http://www.w3.org/2000/svg',defs=document.createElementNS(ns,'defs');g.appendChild(defs);
 cfg.routes.forEach(r=>{verdantEnsureCells(r);for(const state of ['suspected','probable','confirmed']){const cells=r.cells.filter(c=>verdantCellState(c)===state);if(!cells.length)continue;const clip=document.createElementNS(ns,'clipPath'),cid='vrd-'+r.id+'-'+state;clip.id=cid;for(const c of cells){const q=document.createElementNS(ns,'circle');q.setAttribute('cx',c.x);q.setAttribute('cy',c.y);q.setAttribute('r','15');clip.appendChild(q)}defs.appendChild(clip);const im=document.createElementNS(ns,'image');im.setAttribute('href',r.asset);im.setAttribute('x','0');im.setAttribute('y','0');im.setAttribute('width','900');im.setAttribute('height','650');im.setAttribute('preserveAspectRatio','none');im.setAttribute('clip-path','url(#'+cid+')');im.setAttribute('class','verdant-smart-route state-'+state);im.dataset.route=r.id;g.appendChild(im)}
  const rs=verdantStateFor(r);if(rs!=='unassessed'){const tx=document.createElementNS(ns,'text');tx.setAttribute('x',r.label[0]);tx.setAttribute('y',r.label[1]);tx.setAttribute('class','verdant-smart-label state-'+rs);tx.textContent=r.id;g.appendChild(tx)}});
}
...
function verdantSmartTick(dt){if(!verdantSmartActive||activeOA!=='VERDANT')return;const cfg=window.CP_VERDANT_MAP?.routeFoundation;if(!cfg)return;let routesInRange=0,changed=false;const radius=118,r2=radius*radius;
 cfg.routes.forEach(r=>{verdantEnsureCells(r);let hit=false;const before=verdantStateFor(r);for(const c of r.cells){const dx=c.x-A.x,dy=c.y-A.y;if(dx*dx+dy*dy<=r2){hit=true;const old=verdantCellState(c);c.progress+=dt;const now=verdantCellState(c);if(old!==now)changed=true}}if(hit)routesInRange++;const after=verdantStateFor(r);if(after!==before){r.state=after;const m=verdantRouteMetrics(r);cpNotice('ROUTE INTELLIGENCE // '+r.id,after.toUpperCase()+' // '+Math.round(m.coverage*100)+'% COVERAGE','intel');log('VERDANT // '+r.id+' // '+after.toUpperCase()+' // '+Math.round(m.coverage*100)+'%')}});
 if(changed)cpRenderVerdantRoutes();const rl=$('#rangeLine');if(rl)rl.textContent='SENSOR // '+routesInRange+' ROUTE'+(routesInRange===1?'':'S')+' // SPATIAL COLLECTION ACTIVE';verdantUpdateHud();}
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small'),map=$('#map');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(map)map.setAttribute('preserveAspectRatio',activeOA==='VERDANT'?'none':'xMidYMid meet');if(brand)brand.textContent='v0.11.6.32 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpVerdantRaster();cpRenderStrategicPriorityNAI();cpRenderVerdantRoutes();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();cpVerdantHexAspect();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();cpVerdantHexAspect()})});}
