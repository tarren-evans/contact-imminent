...
function ciRenderPhysicalGrid(){const host=document.getElementById('mapWrap'),g=document.getElementById('physicalGrid');if(!host||!g)return;const w=Math.max(1,host.clientWidth-12),h=Math.max(1,host.clientHeight-12),scale=Math.min(w/900,h/650),padX=(w-900*scale)/2,padY=(h-650*scale)/2,r=52*scale,dx=78*scale,dy=Math.sqrt(3)*52*scale;g.setAttribute('viewBox',`0 0 ${w} ${h}`);g.setAttribute('preserveAspectRatio','none');g.replaceChildren();const pts=(cx,cy)=>Array.from({length:6},(_,i)=>{const a=Math.PI*i/3;return `${(cx+r*Math.cos(a)).toFixed(2)},${(cy+r*Math.sin(a)).toFixed(2)}`}).join(' ');let c0=Math.floor((-r-padX)/dx)-1,c1=Math.ceil((w+r-padX)/dx)+1;for(let c=c0;c<=c1;c++){const cx=padX+c*dx,off=(((c%2)+2)%2)*dy/2;let y0=Math.floor((-r-padY-off)/dy)-1,y1=Math.ceil((h+r-padY-off)/dy)+1;for(let row=y0;row<=y1;row++){const poly=document.createElementNS(NS,'polygon');poly.setAttribute('points',pts(cx,padY+off+row*dy));poly.setAttribute('fill','none');poly.setAttribute('stroke','rgba(224,233,236,.42)');poly.setAttribute('stroke-width','1.45');poly.setAttribute('vector-effect','non-scaling-stroke');poly.setAttribute('pointer-events','none');g.appendChild(poly)}}}
let ciGridRAF=0;function ciSchedulePhysicalGrid(){cancelAnimationFrame(ciGridRAF);ciGridRAF=requestAnimationFrame(ciRenderPhysicalGrid)}ciSchedulePhysicalGrid();window.addEventListener('resize',ciSchedulePhysicalGrid,{passive:true});window.visualViewport?.addEventListener('resize',ciSchedulePhysicalGrid,{passive:true});function renderTerrain(){
 ...
 // v0.6.6.16: terrain imagery lives on #mapWrap as overscan. The SVG now
