...
function cpEffectiveISRSpeed(){normalizeISRCapability();return A.speed*(isrCapability==='RAPID'?1.12:1)}
...
const sites=activeOA==='VANTAGE'&&window.CP_VANTAGE_MAP?window.CP_VANTAGE_MAP.blufor:[{id:'BLU-FOB',x:705,y:485},{id:'BLU-C2',x:600,y:170},{id:'BLU-LOG',x:300,y:310},{id:'BLU-AIR',x:775,y:285},{id:'BLU-AOB',x:425,y:205}];
...
function usesObjectiveTracker(){return isCampaignMission()||campaignMission==='ARCADE'}
...
function renderLKP(t){if(intelAssistMode()==='OFF'){if(t.lkp?.g)t.lkp.g.setAttribute('visibility','hidden');return}
 ...
 if(show){
  ...
  l.g.setAttribute('transform',`translate(${i.lastX} ${i.lastY})`);l.g.setAttribute('opacity',String(style.op));
...
function cpEffectiveISRRadius(){normalizeISRCapability();return A.r*(isrCapability==='WIDE'?1.10:1)*(cpFusionActive()?1.30:1)}
...
if(campaignMission&&or){
 ...
 if(oc)oc.textContent=Math.round(campaignCompromise)+'/'+((campaignMission==='01-05'||campaignMission==='02-05')?'50':'60');
...
log('MISSION START // CONTACT IMMINENT 0.11.5.14 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(brand)brand.textContent='v0.11.5.14.1 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpRenderStrategicPriorityNAI();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();requestAnimationFrame(()=>window.ciPhysicalPhoneMapFit?.())});}
...
function cpVerdantPreview(){clearTutorialState();running=false;stopCollect();window.CP_MENU=true;cpSetOAVisual('VERDANT');$('#campaignMenu').classList.add('hidden');$('#mainMenu').classList.add('hidden');$('#verdantPreviewHud').classList.remove('hidden');$('#phase').textContent='OA VERDANT // PREVIEW';$('#clock').textContent='--:--';}
function cpExitVerdantPreview(){$('#verdantPreviewHud').classList.add('hidden');cpSetOAVisual('KESTREL');cpCampaignMenu();}
...
function cpExitMiragePreview(){$('#miragePreviewHud').classList.add('hidden');cpSetOAVisual('KESTREL');cpCampaignMenu();}
...
function cpExitTempestPreview(){$('#tempestPreviewHud').classList.add('hidden');cpSetOAVisual('KESTREL');cpCampaignMenu();}
...
function c02Unlocked(id){let n=Number(id.slice(-1));if(n===1)return localStorage.getItem('ci-c01-complete')==='1';return localStorage.getItem('ci-c02-0'+(n-1)+'-complete')==='1'}
...
function setArcadeOA(oa){arcadeOA=oa==='VANTAGE'?'VANTAGE':'KESTREL';localStorage.setItem(ARCADE_OA_KEY,arcadeOA);document.querySelectorAll('[data-arcade-oa]').forEach(b=>b.classList.toggle('selected',b.dataset.arcadeOa===arcadeOA));let l=$('#arcadeOALabel');if(l)l.textContent=arcadeOA}
...
const GLOBAL_OA={KESTREL:'MOUNTAIN-COAST // FOUNDATION',VANTAGE:'URBAN-INDUSTRIAL // NETWORK',VERDANT:'TROPICAL RIVER-COAST // PREVIEW',MIRAGE:'ARID ROUTE-LOGISTICS // PREVIEW',TEMPEST:'MARITIME ARCHIPELAGO // PREVIEW',ARCTIC:'POLARIS // POLAR ARCHIPELAGO / ICE / MOUNTAIN CORRIDORS'};
function openGlobal(){window.CP_MENU=true;globalSelectedOA=null;$('#mainMenu').classList.add('hidden');$('#globalMenu').classList.remove('hidden');document.querySelectorAll('[data-global-oa]').forEach(b=>b.classList.remove('selected'));$('#globalOAName').textContent='NO OA SELECTED';$('#globalOADetail').textContent='Select an operational-area marker on the globe to inspect the regional picture.';$('#globalCampaign').disabled=true;$('#globalArcade').disabled=true;$('#globalTraining').disabled=true;$('#globalPlanNAI').disabled=true;}
function closeGlobal(){$('#globalMenu').classList.add('hidden');$('#mainMenu').classList.remove('hidden');rotateCommandBackground();}
function selectGlobalOA(oa){globalSelectedOA=oa;document.querySelectorAll('[data-global-oa]').forEach(b=>b.classList.toggle('selected',b.dataset.globalOa===oa));const num=GLOBAL_OA_NUM[oa]||'--';$('#globalOAName').textContent='OA '+num+' // '+(oa==='ARCTIC'?'POLARIS':oa);$('#globalOADetail').textContent=(GLOBAL_OA[oa]||oa)+' // REGIONAL PICTURE AVAILABLE';const campaignReady=['KESTREL','VANTAGE'].includes(oa),arcadeReady=['KESTREL','VANTAGE'].includes(oa);$('#globalCampaign').disabled=!campaignReady;$('#globalArcade').disabled=!arcadeReady;$('#globalTraining').disabled=oa==='ARCTIC';$('#globalPlanNAI').disabled=false;refreshGlobalTaskingDetail(oa);}

let globalActivityContext=false;
function globalCampaignActivity(){if(!globalSelectedOA||!['KESTREL','VANTAGE'].includes(globalSelectedOA))return;globalActivityContext=true;campaignMenuOA=globalSelectedOA;closeGlobal();cpCampaignMenu();document.body.classList.add('global-context-campaign');}
function globalArcadeActivity(){if(!globalSelectedOA||!['KESTREL','VANTAGE'].includes(globalSelectedOA))return;globalActivityContext=true;setArcadeOA(globalSelectedOA);closeGlobal();arcadeMenu();document.body.classList.add('global-context-arcade');}
...
// v0.11.5.14.1 // GLOBAL GLOBE FOUNDATION
...
const GLOBAL_OA_DISPLAY={KESTREL:'KESTREL',VANTAGE:'VANTAGE',VERDANT:'VERDANT',MIRAGE:'MIRAGE',TEMPEST:'TEMPEST',ARCTIC:'POLARIS'};
const GLOBAL_OA_NUM={KESTREL:'01',VANTAGE:'02',VERDANT:'03',MIRAGE:'04',TEMPEST:'05',ARCTIC:'06'};
const GLOBAL_ORBITS=[{inc:52,raan:-28,phase:15,period:24},{inc:67,raan:66,phase:142,period:34},{inc:31,raan:154,phase:254,period:19}];
...
// v0.11.5.14 // MOBILE GLOBAL PERFORMANCE PROFILE
...
const GLOBAL_SPACE_FRAME_MS=GLOBAL_MOBILE?1000/12:0;
const GLOBAL_CLOUD_STEP=GLOBAL_MOBILE?2:1;
let globalLastFrame=0,globalLastSpaceFrame=0;
let globalSpaceEnv=null;
function initGlobalSpaceEnv(){
 const scene=$('#orbitalScene'),stars=$('#globalSpaceCanvas'),dust=$('#globalDustCanvas');if(!scene||!stars||!dust||globalSpaceEnv)return;
 let seed=7319,rand=()=>((seed=(seed*1664525+1013904223)>>>0)/4294967296);
 globalSpaceEnv={stars:Array.from({length:180},()=>({x:rand(),y:rand(),r:.35+rand()*1.25,a:.18+rand()*.68,d:.18+rand()*.82})),dust:Array.from({length:42},()=>({x:rand(),y:rand(),r:12+rand()*46,a:.012+rand()*.04,d:.12+rand()*.35}))};
}
function drawGlobalSpace(now){
 if(GLOBAL_SPACE_FRAME_MS&&now-globalLastSpaceFrame<GLOBAL_SPACE_FRAME_MS)return;globalLastSpaceFrame=now;
 initGlobalSpaceEnv();const scene=$('#orbitalScene'),sc=$('#globalSpaceCanvas'),dc=$('#globalDustCanvas');if(!scene||!sc||!dc||!globalSpaceEnv)return;
 const rect=scene.getBoundingClientRect(),dpr=GLOBAL_RENDER_DPR,W=Math.max(2,Math.round(rect.width*dpr)),H=Math.max(2,Math.round(rect.height*dpr));
 [sc,dc].forEach(c=>{if(c.width!==W||c.height!==H){c.width=W;c.height=H}});
 const ctx=sc.getContext('2d'),dtx=dc.getContext('2d'),t=now*.00001;ctx.clearRect(0,0,W,H);dtx.clearRect(0,0,W,H);
 globalSpaceEnv.stars.forEach(p=>{let x=((p.x+t*p.d)%1)*W,y=((p.y+Math.sin(t*7+p.x*9)*.0015*p.d)%1)*H;ctx.beginPath();ctx.arc(x,y,p.r*dpr,0,Math.PI*2);ctx.fillStyle='rgba(196,224,232,'+p.a+')';ctx.fill()});
 globalSpaceEnv.dust.forEach(p=>{let x=((p.x-t*p.d*.25+1)%1)*W,y=p.y*H,rr=p.r*dpr;let gr=dtx.createRadialGradient(x,y,0,x,y,rr);gr.addColorStop(0,'rgba(93,142,164,'+p.a+')');gr.addColorStop(1,'rgba(40,74,92,0)');dtx.fillStyle=gr;dtx.fillRect(x-rr,y-rr,rr*2,rr*2)});
 // Deep-space mood: broad procedural Milky Way band + faint nebular dust.
 ctx.save();ctx.globalCompositeOperation='screen';
 ctx.translate(W*.50,H*.48);ctx.rotate(-.34);
 const mw=ctx.createLinearGradient(0,-H*.42,0,H*.42);
 mw.addColorStop(0,'rgba(34,68,88,0)');mw.addColorStop(.28,'rgba(56,92,116,.035)');
 mw.addColorStop(.45,'rgba(112,137,150,.075)');mw.addColorStop(.52,'rgba(170,181,181,.10)');
 mw.addColorStop(.60,'rgba(78,107,128,.065)');mw.addColorStop(.78,'rgba(44,72,96,.025)');mw.addColorStop(1,'rgba(20,40,58,0)');
 ctx.fillStyle=mw;ctx.fillRect(-W*.75,-H*.48,W*1.5,H*.96);
 for(let i=0;i<150;i++){const u=((i*73)%149)/149-.5,v=(((i*47)%151)/151-.5)*.42,xx=u*W*1.35,yy=v*H+Math.sin(i*1.71)*H*.035,rr=.35+((i*19)%13)/10;ctx.fillStyle='rgba(190,211,218,'+(.018+((i*29)%17)/850)+')';ctx.beginPath();ctx.arc(xx,yy,rr*dpr,0,Math.PI*2);ctx.fill();}
 ctx.restore();
 // Layered deep-space structure: faint galactic core, dust lanes, blue/cyan nebular haze.
 ctx.save();ctx.globalCompositeOperation='screen';
 const gx=W*.46,gy=H*.50;
 const halo=ctx.createRadialGradient(gx,gy,0,gx,gy,Math.max(W,H)*.62);
 halo.addColorStop(0,'rgba(126,157,171,.075)');halo.addColorStop(.22,'rgba(54,94,119,.050)');
 halo.addColorStop(.55,'rgba(24,61,85,.024)');halo.addColorStop(1,'rgba(0,0,0,0)');
 ctx.fillStyle=halo;ctx.fillRect(0,0,W,H);
 ctx.translate(W*.50,H*.50);ctx.rotate(-.34);
 for(let band=0;band<5;band++){
   const yy=(band-2)*H*.055,grad=ctx.createLinearGradient(0,yy-H*.08,0,yy+H*.08);
   grad.addColorStop(0,'rgba(20,42,58,0)');grad.addColorStop(.42,'rgba(82,111,129,'+(0.018+band*.006)+')');
   grad.addColorStop(.52,'rgba(153,166,171,'+(0.025+band*.008)+')');grad.addColorStop(.64,'rgba(49,80,105,'+(0.018+band*.005)+')');grad.addColorStop(1,'rgba(10,24,38,0)');
   ctx.fillStyle=grad;ctx.fillRect(-W*.8,yy-H*.10,W*1.6,H*.20);
 }
 ctx.globalCompositeOperation='source-over';
 for(let i=0;i<8;i++){const xx=((i*137)%701)/701*W*1.4-W*.7,yy=((i*83)%257)/257*H*.5-H*.25,rr=H*(.08+(i%3)*.035);
   const dust=ctx.createRadialGradient(xx,yy,0,xx,yy,rr);dust.addColorStop(0,'rgba(0,5,12,.14)');dust.addColorStop(1,'rgba(0,5,12,0)');
   ctx.fillStyle=dust;ctx.beginPath();ctx.arc(xx,yy,rr,0,Math.PI*2);ctx.fill();}
 ctx.restore();
 // Sparse asteroid traffic across the star field.
 const asteroidCycle=(now*.000018)%1;
 for(let i=0;i<5;i++){const phase=(asteroidCycle+i*.217)%1;if(phase>.72)continue;const ax=W*(1.10-phase*1.55),ay=H*(.10+((i*37)%73)/100*.78)+Math.sin(phase*9+i)*H*.025,ar=Math.max(1.4,Math.min(W,H)*(.0022+(i%3)*.0012));ctx.save();ctx.translate(ax,ay);ctx.rotate(phase*11+i);ctx.fillStyle='rgba(123,133,136,.48)';ctx.beginPath();for(let k=0;k<9;k++){const a=k*Math.PI*2/9,rr=ar*(.72+((k*17+i*11)%31)/100),x=Math.cos(a)*rr,y=Math.sin(a)*rr;if(k)ctx.lineTo(x,y);else ctx.moveTo(x,y)}ctx.closePath();ctx.fill();ctx.restore();}
 // Very rare unidentified orbital contact.
 const ufoWindow=(Math.floor(now/1000)%173);if(ufoWindow>=31&&ufoWindow<36){const u=(ufoWindow-31+(now%1000)/1000)/5,ux=W*(-.08+u*1.16),uy=H*(.19+.035*Math.sin(u*Math.PI*2));ctx.save();ctx.translate(ux,uy);ctx.strokeStyle='rgba(148,226,230,.62)';ctx.lineWidth=Math.max(1,dpr*.7);ctx.beginPath();ctx.ellipse(0,0,8*dpr,2.3*dpr,0,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(195,242,238,.52)';ctx.beginPath();ctx.ellipse(0,-1.5*dpr,2.4*dpr,1.3*dpr,0,0,Math.PI*2);ctx.fill();ctx.restore();}

}

// v0.11.5.14.1 // GLOBAL EARTH FOUNDATION
const GLOBAL_EARTH_LANDS=[
 [[72,-168],[61,-148],[58,-126],[49,-117],[44,-101],[30,-96],[20,-105],[12,-91],[7,-79],[18,-72],[29,-80],[40,-68],[52,-59],[61,-74],[70,-104]],
 [[12,-82],[3,-77],[-8,-72],[-18,-65],[-31,-61],[-45,-68],[-55,-73],[-47,-55],[-33,-49],[-18,-44],[-5,-51],[5,-61]],
 [[71,-12],[64,8],[58,28],[51,45],[43,57],[34,69],[25,84],[11,103],[-5,112],[-17,103],[-27,82],[-34,56],[-29,35],[-16,23],[-2,15],[10,4],[24,-8],[36,-12],[49,-4],[61,-17]],
 [[-11,113],[-20,119],[-29,135],[-39,146],[-44,161],[-32,171],[-19,160],[-11,144],[-8,128]],
 [[82,-54],[78,-31],[70,-26],[64,-42],[69,-62]], [[-62,-180],[-66,-120],[-68,-60],[-66,0],[-69,60],[-67,120],[-62,180]]
];
function globePoly(ctx,poly,cx,cy,r,fill,stroke){
 ctx.beginPath();let pen=false,vis=0;
 poly.forEach(([lat,lon])=>{const q=globeProject(lat,lon,cx,cy,r);if(q.z>-.025){vis++;if(!pen){ctx.moveTo(q.x,q.y);pen=true}else ctx.lineTo(q.x,q.y)}else pen=false});
 if(vis>=3){ctx.closePath();ctx.fillStyle=fill;ctx.fill();if(stroke){ctx.strokeStyle=stroke;ctx.stroke()}}
}
function globeTerrainPatch(ctx,lat,lon,rx,ry,cx,cy,r,fill){
 const pts=[];for(let i=0;i<18;i++){let a=i*Math.PI*2/18,w=1+.12*Math.sin(i*2.7+lat*.09);pts.push([lat+Math.sin(a)*ry*w,lon+Math.cos(a)*rx*w/Math.max(.28,Math.cos(globeRad(lat)))])}
 globePoly(ctx,pts,cx,cy,r,fill,null);
}
...
function initGlobalSurfaceGL(){
 ...
 const c=$('#globalSurfaceCanvas');if(!c)return null;
 const gl=c.getContext('webgl',{alpha:true,antialias:true,premultipliedAlpha:false});if(!gl)return null;
 const vs=`attribute vec3 aPos;attribute vec2 aUV;uniform float uYaw;uniform float uPitch;uniform float uScale;varying vec2 vUV;varying float vZ;
 void main(){float cy=cos(uYaw),sy=sin(uYaw),cp=cos(uPitch),sp=sin(uPitch);vec3 q=vec3(aPos.x*cy+aPos.z*sy,aPos.y,-aPos.x*sy+aPos.z*cy);q=vec3(q.x,q.y*cp+q.z*sp,-q.y*sp+q.z*cp);vUV=aUV;vZ=q.z;gl_Position=vec4(q.x*uScale,q.y*uScale,-q.z*.5,1.0);}`;
 const fs=`precision mediump float;uniform sampler2D uTex;varying vec2 vUV;varying float vZ;
 void main(){if(vZ<-0.002)discard;vec4 c=texture2D(uTex,vUV);gl_FragColor=vec4(c.rgb,1.0);}`;
 function sh(type,src){const x=gl.createShader(type);gl.shaderSource(x,src);gl.compileShader(x);return x}
 const pr=gl.createProgram();gl.attachShader(pr,sh(gl.VERTEX_SHADER,vs));gl.attachShader(pr,sh(gl.FRAGMENT_SHADER,fs));gl.linkProgram(pr);
 const verts=[],idx=[],latN=64,lonN=128;
 for(let iy=0;iy<=latN;iy++){const lat=Math.PI/2-iy*Math.PI/latN,cl=Math.cos(lat),yy=-Math.sin(lat);for(let ix=0;ix<=lonN;ix++){const lon=-Math.PI+ix*Math.PI*2/lonN;verts.push(cl*Math.sin(lon),yy,cl*Math.cos(lon),ix/lonN,iy/latN)}}
 for(let iy=0;iy<latN;iy++)for(let ix=0;ix<lonN;ix++){let p=iy*(lonN+1)+ix,q=p+lonN+1;idx.push(p,q,p+1,p+1,q,q+1)}
 ...
 const img=new Image();img.onload=()=>{gl.bindTexture(gl.TEXTURE_2D,tex);gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL,0);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGB,gl.RGB,gl.UNSIGNED_BYTE,img);globalSurfaceGL.ready=true};img.src=GLOBAL_EARTH_TEXTURE_SRC;
 globalSurfaceGL={c,gl,pr,vb,ib,tex,count:idx.length,ready:false};return globalSurfaceGL;
}
function drawGlobalSurfaceGL(W,H,r){
 const o=initGlobalSurfaceGL();if(!o)return;const {c,gl,pr,vb,ib,tex}=o;if(c.width!==W||c.height!==H){c.width=W;c.height=H}
 gl.viewport(0,0,W,H);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);if(!o.ready)return;
 gl.useProgram(pr);gl.bindBuffer(gl.ARRAY_BUFFER,vb);
 const ap=gl.getAttribLocation(pr,'aPos'),au=gl.getAttribLocation(pr,'aUV');gl.enableVertexAttribArray(ap);gl.vertexAttribPointer(ap,3,gl.FLOAT,false,20,0);gl.enableVertexAttribArray(au);gl.vertexAttribPointer(au,2,gl.FLOAT,false,20,12);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,ib);
 gl.uniform1f(gl.getUniformLocation(pr,'uYaw'),globeRad(globalGlobe.yaw));gl.uniform1f(gl.getUniformLocation(pr,'uPitch'),globeRad(globalGlobe.pitch));gl.uniform1f(gl.getUniformLocation(pr,'uScale'),r/(Math.min(W,H)/2));
 gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,tex);gl.uniform1i(gl.getUniformLocation(pr,'uTex'),0);gl.enable(gl.DEPTH_TEST);gl.depthFunc(gl.LEQUAL);gl.drawElements(gl.TRIANGLES,o.count,gl.UNSIGNED_SHORT,0);
}
function drawGlobalGlobe(now){drawGlobalSpace(now);const c=$('#globalGlobeCanvas'),wrap=$('#globalEarthWrap'),ov=$('#globalGlobeOverlay');if(!c||!wrap||!ov)return;initGlobalGlobe();const rect=wrap.getBoundingClientRect(),dpr=GLOBAL_RENDER_DPR,logical=Math.max(2,Math.round(Math.min(rect.width,rect.height)*dpr)),pad=1.72,W=Math.round(logical*pad),H=Math.round(logical*pad);if(c.width!==W||c.height!==H){c.width=W;c.height=H}const ctx=c.getContext('2d'),cx=W/2,cy=H/2,r=logical*.40*globalGlobe.zoom;ctx.clearRect(0,0,W,H);drawGlobalSurfaceGL(W,H,r);ctx.save();ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.clip();
 // Dynamic day/night treatment: opaque globe, stronger terminator, warm fictional night lights.
 const sunX=cx+r*.58,sunY=cy-r*.10;
 const terminator=ctx.createRadialGradient(sunX,sunY,r*.10,sunX,sunY,r*1.30);
 terminator.addColorStop(0,'rgba(118,194,214,.05)');terminator.addColorStop(.43,'rgba(4,14,20,.04)');
 terminator.addColorStop(.56,'rgba(0,5,12,.24)');terminator.addColorStop(.68,'rgba(0,3,9,.62)');
 terminator.addColorStop(.82,'rgba(0,2,7,.86)');terminator.addColorStop(1,'rgba(0,1,5,.95)');
 ctx.fillStyle=terminator;ctx.fillRect(cx-r,cy-r,r*2,r*2);
 const cityCenters=[[43,-121,1],[40,-111,.85],[36,-96,1.25],[31,-82,.82],[22,-62,.95],[8,-49,.78],[-13,-45,.72],[-27,-57,.92],[50,-7,.9],[46,18,1.28],[38,34,1.12],[30,49,.96],[20,61,.84],[9,73,1.18],[-4,86,.88],[-16,101,.72],[38,111,1.38],[29,124,1.15],[16,137,.96],[-5,148,.78],[-23,143,.88],[-35,132,.7],[63,25,.62],[70,41,.48],[56,68,.76],[43,78,.92],[26,91,1.06],[5,113,1.12],[-7,18,.66],[-19,29,.74],[-32,41,.82],[13,-12,.7],[1,-3,.64],[-23,-5,.58]];
 const cityLights=[];cityCenters.forEach((c,ci)=>{cityLights.push([c[0],c[1],c[2]]);const n=10+Math.round(c[2]*10);for(let k=0;k<n;k++){const aa=k*2.399963+ci*.71,rad=1.1+Math.sqrt((k+1)/n)*(3.8+5.2*c[2]);cityLights.push([c[0]+Math.sin(aa)*rad*.62,c[1]+Math.cos(aa)*rad/Math.max(.34,Math.cos(globeRad(c[0]))),.22+.46*c[2]*(1-k/n)]);}for(let k=1;k<=5;k++){const t=k/6,dir=ci%2?1:-1;cityLights.push([c[0]+dir*t*(3.2+ci%5),c[1]+t*(6.5+ci%7),.18+.18*c[2]]);}});
 ctx.save();ctx.globalCompositeOperation='screen';
 cityLights.forEach((p,i)=>{const q=globeProject(p[0],p[1],cx,cy,r*1.002);if(q.z<=.035)return;
   const dx=(q.x-sunX)/r,dy=(q.y-sunY)/r,d=Math.sqrt(dx*dx+dy*dy),darkness=Math.max(0,Math.min(1,(d-.48)/.46));if(darkness<.08)return;
   const weight=p[2]||.35,rr=Math.max(.55,r*(.00115+.00155*weight))*darkness;
   ctx.beginPath();ctx.arc(q.x,q.y,rr*2.6,0,Math.PI*2);ctx.fillStyle='rgba(255,175,74,'+(.06+.13*darkness)+')';ctx.fill();
   ctx.beginPath();ctx.arc(q.x,q.y,rr,0,Math.PI*2);ctx.fillStyle='rgba(255,220,145,'+(.28+.54*darkness)+')';ctx.fill();
   if(i%3===0)for(let k=0;k<4;k++){let a=(i+k*1.91)*1.53,d2=rr*(2.5+k*.75);ctx.beginPath();ctx.arc(q.x+Math.cos(a)*d2,q.y+Math.sin(a)*d2,Math.max(.62,rr*.38),0,Math.PI*2);ctx.fillStyle='rgba(255,196,96,'+(.14+.24*darkness)+')';ctx.fill();}
 });ctx.restore();
 // GLOBAL WEATHER: broad cloud decks, fronts, and propagating storm systems.
 ctx.globalCompositeOperation='screen';
 const cloudR=r*1.018,weatherT=now*.000018;
 const cloudBelts=[
  {lat:68,lon:-170,n:48,span:118,w:11,a:.14,spd:.32},{lat:57,lon:-118,n:58,span:142,w:14,a:.16,spd:.40},
  {lat:44,lon:-58,n:64,span:158,w:16,a:.18,spd:.48},{lat:31,lon:12,n:62,span:154,w:15,a:.17,spd:.55},
  {lat:18,lon:78,n:68,span:172,w:14,a:.17,spd:.64},{lat:5,lon:145,n:72,span:188,w:13,a:.16,spd:.70},
  {lat:-8,lon:-118,n:68,span:176,w:14,a:.16,spd:.68},{lat:-21,lon:-42,n:64,span:164,w:16,a:.17,spd:.59},
  {lat:-35,lon:36,n:60,span:152,w:16,a:.17,spd:.50},{lat:-49,lon:112,n:56,span:142,w:14,a:.16,spd:.42},
  {lat:-62,lon:174,n:48,span:120,w:12,a:.14,spd:.34}
 ];
 function cloudPuff(lat,lon,size,alpha,seed){
   const q=globeProject(lat,lon,cx,cy,cloudR);if(q.z<=.018)return;
   const depth=.32+.68*Math.min(1,q.z),rr=r*size*depth;
   const cg=ctx.createRadialGradient(q.x-rr*.16,q.y-rr*.18,rr*.03,q.x,q.y,rr*1.12);
   cg.addColorStop(0,'rgba(255,255,255,'+(alpha*1.38)+')');
   cg.addColorStop(.24,'rgba(248,252,252,'+(alpha*1.16)+')');
   cg.addColorStop(.64,'rgba(226,240,242,'+(alpha*.62)+')');
   cg.addColorStop(1,'rgba(205,228,233,0)');
   ctx.fillStyle=cg;ctx.beginPath();ctx.arc(q.x,q.y,rr*1.12,0,Math.PI*2);ctx.fill();
 }
 const macroDecks=[
  {lat:61,lon:-150,rx:36,ry:13,n:90,a:.105,spd:.30},{lat:46,lon:-72,rx:44,ry:17,n:118,a:.115,spd:.40},
  {lat:28,lon:4,rx:38,ry:16,n:104,a:.105,spd:.50},{lat:12,lon:73,rx:46,ry:17,n:126,a:.11,spd:.61},
  {lat:-3,lon:150,rx:48,ry:15,n:130,a:.105,spd:.66},{lat:-19,lon:-112,rx:42,ry:17,n:112,a:.11,spd:.57},
  {lat:-36,lon:-35,rx:43,ry:18,n:116,a:.11,spd:.46},{lat:-51,lon:86,rx:38,ry:15,n:96,a:.10,spd:.36}
 ];
 macroDecks.forEach((d,di)=>{for(let k=0;k<d.n;k+=GLOBAL_CLOUD_STEP){const golden=k*2.3999632297,rad=Math.sqrt((k+.45)/d.n),lat=d.lat+Math.sin(golden+di)*d.ry*rad+Math.sin(weatherT*8+k*.17)*1.5,lon=d.lon+Math.cos(golden+di)*d.rx*rad+weatherT*d.spd*360,edge=1-rad*.44,sz=(.0135+(k%9)*.00115)*(.82+edge*.34);cloudPuff(lat,lon,sz,d.a*(.78+edge*.34),di*10000+k);}});
  cloudBelts.forEach((f,fi)=>{
   for(let k=0;k<f.n;k+=GLOBAL_CLOUD_STEP){
     const u=k/(f.n-1)-.5,phase=k*1.91+fi*2.17;
     const lon=f.lon+u*f.span+weatherT*f.spd*360;
     const lat=f.lat+Math.sin(u*Math.PI*2.2+fi)*f.w*.48+Math.sin(weatherT*7+phase)*2.1;
     const size=.010+(k%7)*.0019+(Math.sin(phase)*.0015);
     const ca=f.a*(.80+.20*Math.sin(phase*1.3)**2);
     cloudPuff(lat,lon,size*1.34,ca,phase);
     cloudPuff(lat+Math.cos(phase)*2.0,lon+Math.sin(phase)*2.8,size*1.08,ca*.88,phase+1);
     cloudPuff(lat-Math.sin(phase)*2.5,lon+Math.cos(phase)*3.5,size*.94,ca*.78,phase+2);
     if(k%2===0){
       cloudPuff(lat+Math.cos(phase*.73)*4.2,lon-Math.sin(phase*.81)*5.4,size*.82,ca*.67,phase+3);
       cloudPuff(lat-Math.cos(phase*1.17)*3.4,lon+Math.sin(phase*.64)*4.6,size*.76,ca*.61,phase+4);
     }
     if(k%5===0)cloudPuff(lat+Math.sin(phase*.51)*6.0,lon+Math.cos(phase*.58)*7.2,size*.70,ca*.52,phase+5);if(k%3===0){cloudPuff(lat+Math.sin(phase*1.41)*5.1,lon-Math.cos(phase*.93)*6.1,size*.66,ca*.48,phase+6);cloudPuff(lat-Math.cos(phase*.77)*5.7,lon-Math.sin(phase*1.22)*6.8,size*.61,ca*.43,phase+7);}
   }
 });
 // Long frontal bands.
 const fronts=[
   {lat:52,lon:-148,len:112,tilt:-.18,spd:.38,a:.16},{lat:43,lon:-35,len:118,tilt:-.24,spd:.50,a:.17},
   {lat:29,lon:58,len:104,tilt:.16,spd:.55,a:.16},{lat:12,lon:146,len:118,tilt:-.12,spd:.61,a:.16},
   {lat:-18,lon:-108,len:110,tilt:-.20,spd:.52,a:.16},{lat:-31,lon:76,len:122,tilt:.19,spd:.43,a:.16},
   {lat:-47,lon:158,len:108,tilt:.24,spd:.37,a:.15}
 ];
 fronts.forEach((f,fi)=>{for(let k=0;k<52;k+=GLOBAL_CLOUD_STEP){const u=k/51-.5,lon=f.lon+u*f.len+weatherT*f.spd*360,lat=f.lat+u*f.len*f.tilt+Math.sin(k*.72+weatherT*11+fi)*2.4;cloudPuff(lat,lon,.013+(k%5)*.0019,f.a,fi*100+k);if(k%2===0)cloudPuff(lat+Math.sin(k*.9)*3.1,lon+Math.cos(k*.7)*3.8,.010+(k%3)*.0017,f.a*.72,fi*500+k);}});
 // Propagating cyclonic systems with spiral arms and dense cores.
 const storms=[
   {lat:24,lon:-54,spd:.62,rot:1,scale:1.18},{lat:-16,lon:118,spd:.48,rot:-1,scale:1.08},
   {lat:48,lon:36,spd:.37,rot:1,scale:.92},{lat:12,lon:164,spd:.58,rot:-1,scale:1.02},
   {lat:-38,lon:-28,spd:.41,rot:1,scale:.94}
 ];
 storms.forEach((st,si)=>{
   const centerLon=st.lon+weatherT*st.spd*360;
   for(let arm=0;arm<4;arm++)for(let k=0;k<38;k+=GLOBAL_CLOUD_STEP){
     const rad=1.1+k*.39*st.scale,a=st.rot*(k*.34+weatherT*18)+arm*Math.PI*2/4;
     const lat=st.lat+Math.sin(a)*rad*.48,lon=centerLon+Math.cos(a)*rad/Math.max(.34,Math.cos(globeRad(st.lat)));
     cloudPuff(lat,lon,.009+(k%4)*.0017,.17+(1-k/38)*.10,si*1000+arm*100+k);
   }
   for(let k=0;k<28;k+=GLOBAL_CLOUD_STEP){const a=k*2.399963+weatherT*9,rad=Math.sqrt((k+.4)/28)*4.6*st.scale;cloudPuff(st.lat+Math.sin(a)*rad,centerLon+Math.cos(a)*rad,.0105+(k%3)*.0018,.20,si*2000+k);}
 });
  // Polar aurora: broad luminous vertical curtains, geographically attached.
 ctx.save();ctx.globalCompositeOperation='screen';
 [{lat:70,sgn:1},{lat:-70,sgn:-1}].forEach((au,ai)=>{
   for(let k=0;k<26;k+=GLOBAL_CLOUD_STEP){
     const lon=-176+k*14+weatherT*(ai?72:-64),lat=au.lat+Math.sin(k*.63+weatherT*19)*2.8;
     const q=globeProject(lat,lon,cx,cy,r*1.026);if(q.z<=.05)continue;
     const h=r*(.045+.035*(.5+.5*Math.sin(k*1.71+weatherT*23)))*q.z;
     const w=Math.max(1.5,r*.009*q.z);
     const ag=ctx.createLinearGradient(q.x,q.y-h*au.sgn,q.x,q.y);
     ag.addColorStop(0,'rgba(92,235,190,0)');ag.addColorStop(.32,'rgba(91,226,181,.09)');
     ag.addColorStop(.72,'rgba(94,215,181,.18)');ag.addColorStop(1,'rgba(126,192,235,.025)');
     ctx.fillStyle=ag;ctx.fillRect(q.x-w*.5,Math.min(q.y,q.y-h*au.sgn),w,Math.abs(h));
   }
 });ctx.restore();
 ctx.globalCompositeOperation='source-over';
 ctx.restore();let atm=ctx.createRadialGradient(cx,cy,r*.92,cx,cy,r*1.075);atm.addColorStop(0,'rgba(85,181,215,0)');atm.addColorStop(.72,'rgba(93,191,224,.12)');atm.addColorStop(.9,'rgba(116,211,238,.24)');atm.addColorStop(1,'rgba(116,211,238,0)');ctx.beginPath();ctx.arc(cx,cy,r*1.08,0,Math.PI*2);ctx.fillStyle=atm;ctx.fill();ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.strokeStyle='rgba(143,218,235,.58)';ctx.lineWidth=1.5*dpr;ctx.stroke();const rate=Number(getComputedStyle(document.documentElement).getPropertyValue('--global-rate')||1),dt=Math.min(.05,(now-globalGlobe.lastT)/1000);globalGlobe.lastT=now;if(!globalGlobe.drag)globalGlobe.yaw-=dt*.8*rate;globalGlobe.orbitT+=dt*rate;const ovRect=ov.getBoundingClientRect(),sx=ovRect.width/W,sy=ovRect.height/H;Object.entries(GLOBAL_OA_GEO).forEach(([oa,ll])=>{let q=globeProject(ll.lat,ll.lon,cx,cy,r),el=ov.querySelector('[data-oa="'+oa+'"]');if(el){el.style.left=(q.x*sx)+'px';el.style.top=(q.y*sy)+'px';el.classList.toggle('callout-left',q.x<cx);el.classList.toggle('callout-right',q.x>=cx);el.classList.toggle('backside',q.z<=0);el.classList.toggle('selected',globalSelectedOA===oa)}});GLOBAL_ORBITS.forEach((o,i)=>{let ll=globeOrbitLatLon(o,globalGlobe.orbitT),q=globeProject(ll.lat,ll.lon,cx,cy,r),el=ov.querySelector('.globe-sat-'+i);if(el){el.style.left=(q.x*sx)+'px';el.style.top=(q.y*sy)+'px';el.classList.toggle('backside',q.z<=0)}})}
...
const eventPanel=$('#eventPanel'),devPanel=$('#devPanel');if(eventPanel){eventPanel.open=true;$('#utilityEventHost')?.appendChild(eventPanel)}if(devPanel){devPanel.open=false;$('#utilityDevHost')?.appendChild(devPanel)}applyVisualSettings();applyAudioSettings();cpRefreshCampaign();cpRefreshCampaign02();setArcadeOA(arcadeOA);cpShowMenu();(function bootSequence(){const splash=$('#bootSplash'),bar=$('#bootLoadBar'),pct=$('#bootPct'),dur=7000,t0=performance.now();try{backgroundMusic.volume=.34*audioLevels.music*(masterMuted?0:1);const p=backgroundMusic.play();if(p&&p.catch)p.catch(()=>{})}catch(e){}function tick(now){let f=Math.min(1,(now-t0)/dur),n=Math.round(f*100);if(bar){bar.style.width=n+'%';bar.style.backgroundColor=n<50?'#ff4b45':n<80?'#e0b23d':'#55d67a'}if(pct){pct.textContent=n+'%';pct.style.color=n<50?'#ff6b65':n<80?'#e0b23d':'#55d67a'}if(f<1)requestAnimationFrame(tick);else splash?.classList.add('done')}requestAnimationFrame(tick)})();
