...
log('MISSION START // CONTACT IMMINENT 0.11.6.10 // OA RUNTIME')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':campaignMission==='TUTORIAL'?'TRAIN':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=['VANTAGE','VERDANT','MIRAGE','TEMPEST','ARCTIC'].includes(oa)?oa:'KESTREL';['vantage','verdant','mirage','tempest'].forEach(k=>w.classList.toggle('oa-'+k,activeOA===k.toUpperCase()));if(brand)brand.textContent='v0.11.6.10 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();cpRenderStrategicPriorityNAI();requestAnimationFrame(()=>{window.ciPhysicalPhoneMapFit?.();requestAnimationFrame(()=>window.ciPhysicalPhoneMapFit?.())});}
...
// v0.11.6.10 // LEAN LIVE GLOBAL GLOBE
...
const GLOBAL_DESCENT_IMAGE={KESTREL:'assets/maps/oa-kestrel-overscan.png',VANTAGE:'assets/maps/oa-vantage-overscan.png',VERDANT:'assets/maps/oa-verdant-overscan.png',MIRAGE:'assets/maps/oa-mirage-overscan.png',TEMPEST:'assets/maps/oa-tempest-overscan.png',ARCTIC:'assets/maps/oa-arctic-basemap.webp'};
const GLOBAL_MAP_IMAGE={KESTREL:'assets/maps/oa-kestrel-basemap.webp',VANTAGE:'assets/maps/oa-vantage-basemap.webp',VERDANT:'assets/maps/oa-verdant-basemap.webp',MIRAGE:'assets/maps/oa-mirage-basemap.webp',TEMPEST:'assets/maps/oa-tempest-basemap.webp',ARCTIC:'assets/maps/oa-arctic-basemap.webp'};
const planningRasterCache={};function ensurePlanningRaster(oa){let img=planningRasterCache[oa];if(img)return img;img=new Image();img.className='planning-map-raster';img.alt='';img.setAttribute('aria-hidden','true');img.decoding='async';img.src=GLOBAL_MAP_IMAGE[oa];planningRasterCache[oa]=img;return img}function mountPlanningRaster(oa){const map=$('#planningMap'),deck=$('#planningWarmDeck');if(!map)return;Object.entries(planningRasterCache).forEach(([key,img])=>{if(key===oa){if(img.parentNode!==map)map.insertBefore(img,map.firstChild);img.classList.add('active')}else{img.classList.remove('active');if(deck&&img.parentNode!==deck)deck.appendChild(img)}})}
...
function openGlobalPlanningDirect(){if(!globalSelectedOA)return;const oa=globalSelectedOA,saved=readGlobalNAIs()[oa];globalPlanningPoints=(saved?.points||[]).map(p=>({x:p.x,y:p.y}));globalPlanningCommitted=!!saved?.committed;globalPlanningDrag=-1;$('#planningOAName').textContent='OA '+({'KESTREL':'01','VANTAGE':'02','VERDANT':'03','MIRAGE':'04','TEMPEST':'05','ARCTIC':'06'}[oa])+' // '+(oa==='ARCTIC'?'POLARIS':oa);const planningMap=$('#planningMap');planningMap.style.backgroundImage="linear-gradient(rgba(4,10,13,.08),rgba(4,10,13,.08)),url('"+GLOBAL_MAP_IMAGE[oa]+"')";mountPlanningRaster(oa);$('#globalPlanning').classList.remove('hidden');renderGlobalPlanning();}
...
const eventPanel=$('#eventPanel'),devPanel=$('#devPanel');if(eventPanel){eventPanel.open=true;$('#utilityEventHost')?.appendChild(eventPanel)}if(devPanel){devPanel.open=false;$('#utilityDevHost')?.appendChild(devPanel)}applyVisualSettings();applyAudioSettings();cpRefreshCampaign();cpRefreshCampaign02();setArcadeOA(arcadeOA);cpShowMenu();(function bootSequence(){
 const splash=$('#bootSplash'),bar=$('#bootLoadBar'),pct=$('#bootPct'),minBoot=1200,t0=performance.now();
 try{backgroundMusic.volume=.34*audioLevels.music*(masterMuted?0:1);const p=backgroundMusic.play();if(p&&p.catch)p.catch(()=>{})}catch(e){}
 const critical=[GLOBAL_EARTH_TEXTURE_SRC,'assets/maps/global-deep-space.webp'];
 function bootPaint(n,label){
   const v=Math.max(0,Math.min(99,n));
   if(bar){bar.style.width=v+'%';bar.style.backgroundColor=v<50?'#ff4b45':v<80?'#e0b23d':'#55d67a'}
   if(pct)pct.textContent=v+'%';
   const k=document.querySelector('.boot-kicker');if(k&&label)k.textContent=label;
 }
 function boundedImage(src,timeout=3500){
   return Promise.race([
     new Promise(resolve=>{
       const img=new Image(),finish=()=>resolve();
       img.onload=()=>{const d=img.decode?.();if(d&&d.then)d.then(finish).catch(finish);else finish()};
       img.onerror=finish;img.src=src;if(img.complete&&img.naturalWidth)finish();
     }),
     new Promise(resolve=>setTimeout(resolve,timeout))
   ]);
 }
 async function deferredPlanningWarmup(){
   const deck=$('#planningWarmDeck');
   for(const [oa] of Object.entries(GLOBAL_MAP_IMAGE)){
     try{
       const img=ensurePlanningRaster(oa);
       if(deck&&img.parentNode!==deck)deck.appendChild(img);
       await Promise.race([
         new Promise(resolve=>{
           const finish=()=>resolve();
           if(img.complete&&img.naturalWidth){const d=img.decode?.();if(d&&d.then)d.then(finish).catch(finish);else finish()}
 ...
 async function warm(){
   bootPaint(8,'COMMAND SYSTEM // LOADING CORE ASSETS');
   await Promise.all(critical.map(src=>boundedImage(src)));
   bootPaint(68,'COMMAND SYSTEM // INITIALIZING GLOBAL RENDERER');
   try{
     const gl=initGlobalSurfaceGL(),limit=performance.now()+1800;
     while(gl&&!gl.ready&&performance.now()<limit)await new Promise(r=>setTimeout(r,32));
     if(gl){drawGlobalSurfaceGL(8,8,3)}
   }catch(e){}
   bootPaint(92,'COMMAND SYSTEM // FINALIZING INTERFACE');
   const wait=Math.max(0,minBoot-(performance.now()-t0));if(wait)await new Promise(r=>setTimeout(r,wait));
   if(bar){bar.style.width='100%';bar.style.backgroundColor='#55d67a'}
   if(pct){pct.textContent='100%';pct.style.color='#55d67a'}
   const k=document.querySelector('.boot-kicker');if(k)k.textContent='TACTICAL COMMAND SYSTEM // READY';
   requestAnimationFrame(()=>requestAnimationFrame(()=>{
     splash?.classList.add('done');
     setTimeout(()=>{deferredPlanningWarmup()},250);
   }));
 }
 warm().catch(()=>{if(bar)bar.style.width='100%';if(pct)pct.textContent='100%';splash?.classList.add('done');setTimeout(()=>{deferredPlanningWarmup()},250)});
