...
function refreshArcadeIntelAssist(){
 ...
 document.querySelectorAll('[data-intel-assist]').forEach(b=>b.classList.toggle('selected',b.dataset.intelAssist===arcadeIntelAssist));
}
...
// v0.7.1.3 shared mission capability layer. Campaign systems query capabilities rather than hard-coding campaign numbers throughout the runtime.
...
function cpSetOAVisual(oa){const w=$('#mapWrap'),brand=document.querySelector('.brand-block small');if(!w)return;activeOA=oa==='VANTAGE'?'VANTAGE':'KESTREL';const vantage=activeOA==='VANTAGE';w.classList.toggle('oa-vantage',vantage);if(brand)brand.textContent='v0.7.1.3 // OA '+activeOA+' // GEOMETRIC GRID';initBlue();}
...
function cpCampaignMenu(){cpSetOAVisual('KESTREL');cpRefreshCampaign02();clearTutorialState();musicSetMode('menu');cpRefreshCampaign();window.CP_MENU=true;window.CP_PAUSED=false;running=false;stopCollect();$('#mainMenu').classList.add('hidden');$('#pauseMenu').classList.add('hidden');$('#briefing0101').classList.add('hidden');$('#briefing0102').classList.add('hidden');$('#briefing0103').classList.add('hidden');$('#briefing0104').classList.add('hidden');$('#briefing0105').classList.add('hidden');$('#campaignResult').classList.add('hidden');$('#campaignMenu').classList.remove('hidden')}
...
document.querySelectorAll('[data-intel-assist]').forEach(b=>b.addEventListener('click',()=>setArcadeIntelAssist(b.dataset.intelAssist)));refreshArcadeIntelAssist();
